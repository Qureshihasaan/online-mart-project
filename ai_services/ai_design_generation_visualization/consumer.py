...
async def consume_product_events() -> None:
    ...
    while True:
        try:
            ...
            logging.info("AI Design consumer started — listening on '%s'", config.KAFKA_PRODUCT_TOPIC)
            break
        ...
        async for message in consumer:
            try:
                ...
                if event_type == "Product_Created":
                    ...
                    logging.info("Product updated: %s", event.get("product", {}).get("product_id"))
                elif event_type == "Product_Deleted":
                    logging.info("Product deleted: %s", event.get("product", {}).get("product_id"))
